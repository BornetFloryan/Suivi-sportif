    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('seances.update', $seance->id) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" value="{{ old('title', $seance->title) }}" class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-4">
                <label class="block font-medium mb-1">Date</label>
                <input type="date" name="date" value="{{ old('date', $seance->date) }}" class="w-full border rounded px-3 py-2">
            </div>

            <div class="mb-6">
                <label class="block font-medium mb-1">Note</label>
                <textarea name="note" class="w-full border rounded px-3 py-2">{{ old('note', $seance->note) }}</textarea>
            </div>
            ...
            <div id="exercices-container">
                @php
                    $exercices = old('exercices', $exercices ?? $seance->exercices->toArray());
                    if (empty($exercices)) {
